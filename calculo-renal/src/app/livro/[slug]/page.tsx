...
export default async function Page({ params }: PageProps) {
    ...
    try {
        let content = await getMDX("src/content/livro", chapter.slug)

        return (
            <section className="grid grid-rows-1 grid-cols-[20%_80%] max-w-screen min-h-0 max-h-full">
                <aside className="flex flex-col gap-8 p-8 shadow-2xl">
                    <div className="flex flex-col gap-2">
                        { prevChapter ?
                            <Link href={`/livro/${prevChapter.slug}`}
                            className="flex text-lg font-heading gap-2">
                                <ArrowLeftIcon width={24}/>
                                <span>{prevChapter.title}</span>
                            </Link> : null
                        }
                        { nextChapter ?
                            <Link href={`/livro/${nextChapter.slug}`}
                            className="flex flex-row-reverse text-lg font-heading gap-2">
